...
class Tutor:
    ...
    def get_responses(self, df_responses, response_name_column):
        if response_name_column in df_responses.columns:
            # Robust matching: convert both sides to string and strip whitespace before comparison
            ...
            if not matches.empty:
                ...
                if is_new_submission_for_cumulative_update:
                    ...
                    # Update lesson count based on the form
                    ...
                    # Update total hours based on the form
                    hours_field = next((k for k in latest_form.index if "hour" in k.lower()), None)
                    if hours_field:
                        try:
                            hours = float(latest_form[hours_field])
                            self.update_hours(hours) # This updates self.total_hours
                            updates["weekly_hours"] = hours # This is for the current week's report
                        except (ValueError, TypeError):
                            updates["weekly_hours"] = "Invalid"
                    self.last_processed_timestamp = form_timestamp # Update last processed timestamp
                else:
                    ...
                    # Still populate weekly_updates for current report even if not new for cumulative
                    taught_field = next((k for k in latest_form.index if "tutor" in k.lower() and "this" in k.lower()), None)
                    if taught_field:
                ...
                # These should always be set based on the latest form for the current report
                self.weekly_hours = updates.get("weekly_hours", 0.0) # Reset weekly_hours for current report
                if isinstance(self.weekly_hours, str): self.weekly_hours = 0.0 # Ensure it is float for sum later
                ...
                self.weekly_hours = 0.0 # Reset weekly_hours if no response
        else:
            print(f"Missing column: '{response_name_column}'\n")
            self.weekly_hours = 0.0 # Reset weekly_hours if column missing
    ...
    def summarize_responses(self):
        ...
        try:
            response_text = ""
            # Skip the first question (tutor name) and only include other responses
            response_items = list(self.google_form_responses.items())
            for key, value in response_items[1:]:  # Skip first item (name question) for student privacy
                if isinstance(value, str) and value.strip():
                    response_text += f"{key.strip()}: {value.strip()}\n"

            # Enhanced prompt that ensures English tutoring focus
            prompt = (
            ...
            data = {
                ...
                "temperature": 0.3,  # Slightly increased temperature for a little more nuance
                "max_tokens": 300
            }
...
def load_tutors():
    ...
    for _, row in df.iterrows():
        # Handle potential empty strings for numeric fields before conversion
        ...
        # Clean string fields during loading
        ...
        tutor = Tutor(
            ...
            grad=row.get("Grad Year", "NA"), # Assuming Grad Year doesn't need stripping or is handled if non-string
            email=str(email_val).strip().lower() if email_val else "NA", # Strip and lowercase email for consistency
...
def attach_lesson_reports_to_tutors(tutors, response_csv_url):
    ...
    # If still not found, try to find any column that contains "name" in it
    if not name_col:
        for col in df.columns:
...
def save_tutors_to_google_sheet(tutors, sheet_url):
    try:
        ...
        data = [[
            ...
            t.lesson_count, "", # Tutee Name placeholder
            ...
            '' if pd.isna(t.last_processed_timestamp) else t.last_processed_timestamp.strftime('%Y-%m-%d %H:%M:%S') # Format timestamp for sheet
        ] for t in tutors]
...
def generate_summary_for_school(school, tutors_in_school):
    ...
    for tutor in tutors_in_school:
        ai_summary_text = tutor.summarize_responses() # Renamed for clarity
    ...
    return summary_lines  # Return list instead of joined string
