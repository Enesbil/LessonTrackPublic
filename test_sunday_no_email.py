...
def test_sunday_no_email():
    ...
    # 1. Load tutors (gets current cumulative totals)
    tutors = load_tutors()
    if not tutors:
        print("No tutors found. Aborting test run.")
        return

    # 2. Attach latest lesson reports (updates tutor objects in memory with latest weekly data and increments cumulative totals)
    print("\\n=== Attaching Lesson Reports ===")
    attach_lesson_reports_to_tutors(tutors, RESPONSE_CSV_URL)

    # 3. Save updated tutor data back to the sheet (persists new cumulative totals and weekly flags)
    print("\\n=== Saving Updated Tutor Data to Google Sheet ===")
    save_tutors_to_google_sheet(tutors, TUTOR_INFO_CSV_URL.replace("/export?format=csv", ""))

    # 4. Generate summaries for each school using the updated tutor data
    print("\\n=== Generating School Summaries ===")
    school_summaries = generate_school_summaries(tutors)

    # 5. Print summaries to console instead of emailing
