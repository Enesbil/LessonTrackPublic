...
# Management team emails from environment variables
MANAGEMENT_EMAILS = [
    os.getenv("MANAGEMENT_EMAIL", "management@yourcompany.com"),  # Main management email
    # Add other management emails here as needed
]
...
def send_email(recipient, subject, body, is_failure_notification=False):
    ...
    # Handle token refresh for unattended operation
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            try:
                ...
                # Check if we're running locally (can do interactive auth) or on GitHub Actions
    ...
    try:
        ...
        if not is_failure_notification:
            ...
            # Call self to send notification; the flag will prevent looping.
            # If this notification send fails, send_email's own try/except above will print that failure,
            # and since is_failure_notification will be True, it won't loop.
            send_email(
                admin_email,
...
def send_weekly_reports():
    ...
    # Save updated tutor data back to the sheet
    ...
    # Add detailed school summaries
    for school, summary in school_summaries.items():
        management_body += f"\n=== {school} ===\n{summary}\n"
    
    # Send to each management team member
...
def Monday_email_to_tutors():
    #Send reminder emails to all tutors on Monday telling them to submit their reports.
    tutors = load_tutors()  # Now using load_tutors from ai_summary.py
